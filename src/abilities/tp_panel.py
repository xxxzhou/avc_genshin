...
# ⚠ 传送按钮 OCR 区域：右下角，避开「传送锚点」面板标题里的「传送」二字。
# 实机标定（2026-08-15 verify OCR）：真按钮 '传送'@(1663,997)，标题「传送锚点」在
# y≈745 —— 旧 ROI y∈[500,780] 恰好反了：包含标题、排除真按钮 → find_teleport_button
# 找不到按钮 → 按 F 兜底未生效 → wait_main_ui_timeout（r_20260815_081653）。
# 现取右下角 y∈[880,1080], x∈[1400,1920]（含 (1663,997)，排除标题）。
_TELEPORT_BUTTON_ROI = (1400, 880, 520, 200)
